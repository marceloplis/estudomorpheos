...
<div class="formulario">
	...
	<html:form action="/secure/gravavisita?method=gravarvisita12" styleId="formV">
		<html:hidden property="visita12.visita.idVisita"/>
		<html:hidden property="visita12.visita.nuVisita" value="12"/>
		<html:hidden property="visita12.visita.paciente.idPaciente" value="${visitasForm.paciente.idPaciente}"/>
		...
		<div class="span10">
			<div class="well well-small">
				<div class="row-fluid">					
					<div class="span10">	
						<div class="inline">
						  	Data da Realização <html:text property="visita12.visita.dtRealizacaoParse" styleClass="input-small" styleId="dt1"/>  						 
						</div>
					</div>				
				</div>			
			</div>		
		...
		<c:if test="${flAcessoModal ne 'S'}">	
			...
			<c:choose>
				<c:when test="${flAcessoAprovador eq 'S' || visitasForm.acessoAprovador eq 'S'}">
					<html:hidden property="visita12.visita.assinaturaAprovador" value="${sessionScope.UsuarioSistema.username}"/>
					<html:hidden property="visita12.visita.stAprovacao" value="A"/>
				...
				<c:otherwise>			
					<c:choose>
						...
						<c:when test="${visitasForm.visita12.visita.stAprovacao eq 'A'}">
							<div class="span10 hero-unit">   
			      				<span class="msg-info">Visita Aprovada por ${visitasForm.visita12.visita.assinaturaAprovador} em <fmt:formatDate type="both" dateStyle="short" timeStyle="short" value="${visitasForm.visita12.visita.dtAssinaturaAprovacao}" /></span>
		   					</div>
						</c:when>
						
						<c:when test="${visitasForm.visita12.visita.stAprovacao eq 'R'}">
							<div class="span10 hero-unit">
			      				<span class="msg-error">Visita Reprovada por ${visitasForm.visita12.visita.assinaturaAprovador} em <fmt:formatDate type="both" dateStyle="short" timeStyle="short" value="${visitasForm.visita12.visita.dtAssinaturaAprovacao}" /></span>
		   					</div>
						</c:when>			
		...
		<c:if test="${UsuarioSistema.perfil.idPerfilAcesso eq 1 && visitasForm.visita12.visita.stAprovacao eq 'A'}">
			<div class="span10 hero-unit">
				<html:link styleClass="btn" action="/secure/visitas?method=desfazerAprovacao&idParam=${visitasForm.visita12.visita.idVisita}" styleId="desfazAprovacao">Desfazer Aprovação</html:link>
				<script>
				...
				<div class="subpanel">
					...
					<div class="acessoRapido HTMLcommon">						
						<div class="acessoRapidoContainer">
							<ul>
								<li><img src="../shared/img/acessar2.png"/><html:link action="/secure/questionarios?method=qpressaoarterial&idVisita=${visitasForm.visita12.visita.idVisita}&nuVisita=${visitasForm.nuVisita}&flAcessoAprovador=${flAcessoAprovador}">Pressão Arterial</html:link></li>
								<hr/>										
							</ul>
						</div>																		
					</div>	
...
<c:choose>
	<c:when test="${flAcessoModal eq 'S' || flAcessoAprovador eq 'S' || visitasForm.acessoAprovador eq 'S' || visitasForm.visita12.visita.stAprovacao eq 'A' || visitasForm.visita12.visita.stAprovacao eq 'R' || visitasForm.paciente.ativo eq 'N'}">
		<script>		
			$(document).ready(function() {
