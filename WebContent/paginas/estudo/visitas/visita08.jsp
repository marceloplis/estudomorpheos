...
<div class="formulario">
	...
	<html:form action="/secure/gravavisita?method=gravarvisita08" styleId="formV">
		<html:hidden property="visita08.visita.idVisita"/>
		<html:hidden property="visita08.visita.nuVisita" value="8"/>
		<html:hidden property="visita08.visita.paciente.idPaciente" value="${visitasForm.paciente.idPaciente}"/>
		...
		<div class="span10">
			<div class="well well-small">
				<div class="row-fluid">					
					<div class="span10">	
						<div class="inline">
						  	Data da Realização <html:text property="visita08.visita.dtRealizacaoParse" styleClass="input-small" styleId="dt1"/>	 						 
						</div>
					</div>				
				</div>			
			</div>		
		...
		<c:if test="${flAcessoModal ne 'S'}">	
			...
			<c:choose>
				<c:when test="${flAcessoAprovador eq 'S' || visitasForm.acessoAprovador eq 'S'}">
					<html:hidden property="visita08.visita.assinaturaAprovador" value="${sessionScope.UsuarioSistema.username}"/>
					<html:hidden property="visita08.visita.stAprovacao" value="A"/>
				...
				<c:otherwise>			
					<c:choose>
						...
						<c:when test="${visitasForm.visita08.visita.stAprovacao eq 'A'}">
							<div class="span10 hero-unit">   
			      				<span class="msg-info">Visita Aprovada por ${visitasForm.visita08.visita.assinaturaAprovador} em <fmt:formatDate type="both" dateStyle="short" timeStyle="short" value="${visitasForm.visita08.visita.dtAssinaturaAprovacao}" /></span>
		   					</div>
						</c:when>
						
						<c:when test="${visitasForm.visita08.visita.stAprovacao eq 'R'}">
							<div class="span10 hero-unit">
			      				<span class="msg-error">Visita Reprovada por ${visitasForm.visita08.visita.assinaturaAprovador} em <fmt:formatDate type="both" dateStyle="short" timeStyle="short" value="${visitasForm.visita08.visita.dtAssinaturaAprovacao}" /></span>
		   					</div>
						</c:when>			
		...
		<c:if test="${UsuarioSistema.perfil.idPerfilAcesso eq 1 && visitasForm.visita08.visita.stAprovacao eq 'A'}">
			<div class="span10 hero-unit">
				<html:link styleClass="btn" action="/secure/visitas?method=desfazerAprovacao&idParam=${visitasForm.visita08.visita.idVisita}" styleId="desfazAprovacao">Desfazer Aprovação</html:link>
				<script>
				...
				<div class="subpanel">
					...
					<div class="acessoRapido HTMLcommon">						
						<div class="acessoRapidoContainer">
							<ul>
								<li><img src="../shared/img/acessar2.png"/><html:link action="/secure/questionarios?method=qpressaoarterial&idVisita=${visitasForm.visita08.visita.idVisita}&nuVisita=${visitasForm.nuVisita}&flAcessoAprovador=${flAcessoAprovador}">Pressão Arterial</html:link></li>
								<hr/>										
							</ul>
						</div>																		
					</div>	
...
<c:choose>
	<c:when test="${flAcessoModal eq 'S' || flAcessoAprovador eq 'S' || visitasForm.acessoAprovador eq 'S' || visitasForm.visita08.visita.stAprovacao eq 'A' || visitasForm.visita08.visita.stAprovacao eq 'R' || visitasForm.paciente.ativo eq 'N'}">
		<script>		
			$(document).ready(function() {
